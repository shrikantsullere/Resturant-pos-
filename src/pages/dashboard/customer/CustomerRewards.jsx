            ...
            {isLoadingCoupons ? (
               ...
               <div className="space-y-4">
                {coupons.filter(c => c.code !== 'HAPPYHOUR' || (happyHourTimeLeft !== null && happyHourTimeLeft > 0)).map(coupon => {
                  ...
                  return (
                  <div key={coupon.code} className="card p-5 bg-surface border-none shadow-xl shadow-slate-100/50 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                    ...
                    <button 
                      ...
                      className="w-full sm:w-auto px-4 py-2.5 bg-slate-50 hover:bg-primary hover:text-white rounded-xl text-[9px] font-black uppercase tracking-widest border border-slate-100 transition-all flex items-center justify-center gap-1.5 shrink-0"
                    >
