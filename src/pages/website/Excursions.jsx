...
import { Plane, Ship, Car, ChevronRight, X, Phone, MapPin, Receipt, CheckCircle2, Minus, Plus, Compass, Tent, Trees } from 'lucide-react';
...
const Excursions = () => {
  ...
  const fetchServices = async () => {
    try {
      ...
      if (response.data?.success) {
        // Filter for excursion-related services
        const excursions = response.data.data.filter(s => 
          (s.category?.toLowerCase().includes('excursion') || s.category?.toLowerCase().includes('tour') || s.category?.toLowerCase().includes('activity')) &&
          s.price > 0 // Ensure it has a price
        );
        setServices(excursions);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const getServiceIcon = (type) => {
    ...
    if (t.includes('sea') || t.includes('water') || t.includes('boat') || t.includes('snorkeling')) return Ship;
    if (t.includes('nature') || t.includes('jungle') || t.includes('mountain')) return Trees;
    ...
    try {
      ...
      if (method === 'QR Code') {
        response = await paymentApi.createQrCode({
          bookingId,
          amount: totalAmount,
          description: `Excursion Booking: ${selectedService.name}`
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] text-slate-800 font-sans pb-20">
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-[#e6f4f1] to-[#f4fafa] py-12 md:py-20 px-6 text-center relative overflow-hidden">
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-[#1e8a75] uppercase tracking-tighter mb-2 md:mb-4">Excursions & Activities</h1>
          <p className="text-[10px] md:text-lg text-[#1e8a75]/70 font-bold uppercase tracking-widest">
            Curated guided tours and local adventures
          </p>
        </div>
      </section>
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 py-12 md:py-20 space-y-6 md:space-y-8">
        {isLoading ? (
           ...
           <div className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest">
             No excursions available right now.
           </div>
        ) : (
          services.map((item) => {
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-surface/80 backdrop-blur-xl border border-gray-100 rounded-full py-3 px-8 shadow-2xl z-40 flex items-center gap-8">
         ...
         <Link to="/transport" className="text-gray-400 font-black text-xs uppercase tracking-widest">Rides</Link>
      </div>
              ...
              <div className="p-6 md:p-8 overflow-y-auto no-scrollbar flex-1">
                ...
                {paymentState === 'idle' ? (
                  ...
                  <div className="flex flex-col items-center justify-center space-y-4 py-16 text-center">
                     ...
                     <p className="text-xs font-bold text-slate-400 uppercase tracking-widest max-w-xs">Your excursion reservation has been paid successfully.</p>
